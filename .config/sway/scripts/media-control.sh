...
max_volume=100
mic_max_volume=100
notification_timeout=2000 # in milliseconds
...
function show_volume_notif {
    ...
    if [[ $show_music_in_volume_indicator == "true" ]]; then
        ...
        notify-send -u low -t $notification_timeout -h string:x-dunst-stack-tag:volume_notif -h int:value:$volume -i "$album_art" "$volume_icon $volume%" "$current_song"
    else
        notify-send -u low -t $notification_timeout -h string:x-dunst-stack-tag:volume_notif -h int:value:$volume "$volume_icon $volume%"
    fi
}
...
function show_brightness_notif {
    ...
    get_brightness_icon
    notify-send -u low -t $notification_timeout -h string:x-dunst-stack-tag:brightness_notif -h int:value:$brightness "$brightness_icon $brightness%"
}
